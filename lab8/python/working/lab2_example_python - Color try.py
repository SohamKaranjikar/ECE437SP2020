...
pixels = np.arange(648*486*3).reshape(486,648,3).astype('uint8')

counterpix = 0

for i in range (0,486,1):
    for j in range (0,647,2):
        if(i%2==0):
            pixels[i][j] = [buf12[i*648*4+j*4],0,0]
            pixels[i][j+1] = [0,buf12[i*648*4+j*4],0]
        else:
            pixels[i][j] = [0,buf12[i*648*4+j*4],0]
            pixels[i][j+1] = [0,0,buf12[i*648*4+j*4]]
