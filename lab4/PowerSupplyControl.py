...
if (oscilloscope_id == -1):
    print("Osciloscope is not powered on or connected to the PC.")    
else:
    print("Osciloscope is connected to the PC.")
...
measured_current_power_supply = np.array([]) # create an empty list to hold our values
...
for v in output_voltage:
         ...
         measured_voltage_power_supply = np.append(measured_voltage_power_supply, measured_voltage_tmp)
         ...
         measured_current_multimeter = np.append(measured_current_multimeter, measured_current_tmp)
...
plt.figure()
plt.plot(output_voltage.astype(float),measured_current_power_supply.astype(float))
plt.title("Applied Volts vs. Measured Supplied Current from Power Supply for Diode")
...
plt.figure()
plt.plot(measured_voltage_power_supply.astype(float),measured_current_power_supply.astype(float))
plt.title("Measured Voltage from Power Supply vs. Measured Supplied Current from Power Supply for Diode")
plt.xlabel("Measured Voltage [V]")
plt.ylabel("Measured Current [A]")
plt.draw()

plt.figure()
plt.plot(measured_voltage_oscilloscope.astype(float),measured_current_multimeter.astype(float))
plt.title("Measured Voltage Oscilloscope Diode vs. Measured Current Multimeter")
plt.xlabel("Measured Voltage [V]")
plt.ylabel("Measured Current [A]")
plt.draw()
